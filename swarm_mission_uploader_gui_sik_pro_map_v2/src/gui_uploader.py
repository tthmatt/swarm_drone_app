...
from tkinter import Tk, StringVar, BooleanVar, DoubleVar, IntVar, ttk, filedialog, Text, END, DISABLED, NORMAL, messagebox, Toplevel, Listbox, SINGLE, TclError
...
class SwarmGUI:
    ...
    def _setup_style(self):
        ...
        try:
            ...
            base_font.configure(family='Helvetica', size=10)

        font_metrics = base_font.actual()
        font_family = font_metrics['family']
        font_size = font_metrics['size']
        ...
        style.configure('Subheading.TLabel', background=surface, foreground=self.colors['text'], font=heading_font)
        ...
        style.configure('TNotebook.Tab', padding=(18,10), background=background, font=heading_font)
        ...
        style.configure('Card.TLabelframe.Label', background=surface, foreground=self.colors['text'], font=heading_font)

        style.configure('Treeview', background=surface, fieldbackground=surface, foreground=self.colors['text'], rowheight=26)
        style.configure('Treeview.Heading', background='#e2e8f0', foreground=self.colors['text'], font=heading_font,
    ...
    def _map_get_background(self, lats, lons):
        ...
        if not region:
            return None
        key=region['key']
        if self._map_background and self._map_background.get('key')==key:
            return self._map_background
        image=self._map_fetch_tile_region(region)
        if image is None:
            self._map_background=None
            return None
        arr=np.asarray(image)
        self._map_background={'image':arr, 'extent':region['extent'], 'key':key}
    ...
    def _map_compute_tile_region(self, lats, lons):
        ...
        lat_min=self._clamp_lat(lat_min-lat_margin)
        lat_max=self._clamp_lat(lat_max+lat_margin)
        if lon_max-lon_min>=350:
            lon_min=-180.0; lon_max=180.0
        else:
            lon_min=max(-180.0, lon_min-lon_margin)
            lon_max=min(180.0, lon_max+lon_margin)

        for zoom in range(17, 1, -1):
            region=self._map_tile_bounds(lon_min, lon_max, lat_min, lat_max, zoom)
            if region['width_tiles']<=4 and region['height_tiles']<=4:
                return region
        return self._map_tile_bounds(lon_min, lon_max, lat_min, lat_max, 2)
    ...
    def _map_fetch_tile(self, zoom, x, y):
        ...
        try:
            ...
            self._map_log_once('tile_fetch', f'[map] Satellite tile download failed (using plain background): {exc}')
            return None
